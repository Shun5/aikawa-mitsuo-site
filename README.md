...
経営者・相川光生氏のオフィシャルサイト。静的HTML1ファイル構成（画像はインライン埋め込み）で、お問い合わせフォームは Google Apps Script（GAS）と連携し、スプレッドシート転記＋メール通知を行う。
...
| `aikawa-contact.gs` | お問い合わせ受信用 GAS コード（リポジトリ管理用の控え） |
| `.gitignore` | Git除外設定 |
...
公開しただけではフォーム送信先が未設定です。`aikawa-contact.gs` 末尾の手順に従い、

1. GASをデプロイして `/exec` URL を取得。
2. `index.html` 内の `var GAS_ENDPOINT = '';` にそのURLを設定。
3. GASの `NOTIFY_TO` に通知先メールアドレスを設定。

を行ってください。未設定の間は、送信時にメール作成画面（mailto）が開くフォールバック動作になります。
